@php
    // Compact Alpine clock shared with the popup; `mode` is 'pomodoro' (down) or 'stopwatch' (up).
    $clock = fn (?string $start, int $work, string $mode = 'pomodoro') => "{
        ...
        get label() { return this.mode === 'stopwatch' ? this.fmt(this.elapsed) : this.fmt(this.remaining); },
    }";
@endphp
...
<div class="flex items-center">
    @if ($runningEntryId)
        ...
        <button
            type="button"
            wire:click="toggle"
            wire:key="pill-{{ $runningEntryId }}-{{ $mode }}"
            ...
            title="{{ $mode === 'stopwatch' ? 'Stopwatch' : 'Pomodoro' }} running — click to open"
        >
            ...
            <span class="font-mono tabular-nums" style="font-size: 20px; font-weight: 700; line-height: 1;" :class="{ 'pomodoro-flash': finished }" x-text="label"></span>
